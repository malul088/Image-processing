...
class ImageApp:
    ...
    def on_slider_change(self, _=None):
        ...
        # --- מדידת זמן ל-מקורי ---
        start_vec = time.time()
        transformed_numpy = warp.warp_image_numpy(self.original_image, angle, sx, sy)
        end_vec = time.time()
        print(f"NumPy Time: {end_vec - start_vec:.4f}s")  # print(f"Loops Time: {end_loop - start_loop:.4f}s")
        ...
        self.current_image = transformed_numpy
        self.show_on_canvas(self.current_image)
    # ------------------------------------------------------------
    # Display Logic
    # ------------------------------------------------------------
